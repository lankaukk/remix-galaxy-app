...
export function ProjectOutcome({
  ...
  return (
    ...
    >
      ...
      {outcome && (
        <p className="text-base leading-relaxed mb-6" data-testid="text-outcome">
          {outcome}
        </p>
      )}
          ...
          <div>
            ...
            <ul className="list-disc list-outside ml-5 space-y-2" data-testid="list-usage-data">
              {usageData.map((data, index) => (
                <li key={index} className="text-base leading-relaxed pl-1" data-testid={`usage-data-${index}`} dangerouslySetInnerHTML={{ __html: data }}>
                </li>
              ))}
            </ul>
          </div>
